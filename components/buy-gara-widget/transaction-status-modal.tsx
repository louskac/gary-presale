...
export default function TransactionStatusModal({ open, setOpen, toggleOpen, senderChainTxUrl }) {
  ...
  return (
    <Dialog onOpenChange={toggleOpen} open={open}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="sr-only text-foreground">Transaction</DialogTitle>
        ...
        <div className="grid gap-6">
          <div>
            <div className="mb-12">
              {isPending ? (
                <div className="flex h-14 w-full items-center justify-center rounded-full bg-yellow-500 text-foreground">
                  <Loader2Icon className="mx-auto size-10 animate-spin" />
                </div>
              ) : null}
              {hasFinished ? (
                <div className="flex h-14 w-full items-center justify-center rounded-full bg-green-500 text-foreground">
